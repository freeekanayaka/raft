...
void membershipLeadershipTransferInit(struct raft *r, raft_id id)
{
    r->transfer.id = id;
    r->transfer.start = r->clock->now(r->clock);
    r->transfer.timeout_sent = false;
}
