...
int tickLeader(struct raft *r)
{
     ...
     *   timeouts.
     */
    replicationHeartbeat(r, false /* initial */);
...
void tickMaybeExpireTransfer(struct raft *r)
{
    raft_time now;
    ...
    if (now - r->transfer.start >= r->election_timeout) {
        tracef("transfer to %llu pending since %llu msecs -> expire",
               r->transfer.id, now - r->transfer.start);
